...
function onestep_rs_deg3(A::MPEM2, pᵢ⁰, wᵢ, ϕᵢ; ε=1e-6,
...
function iterate_rs_deg3(A::MPEM2, pᵢ⁰, wᵢ, ϕᵢ; ε=1e-6, maxiter=5, tol=1e-10,
        Δs = zeros(0), verbose=true, showprogress=verbose)
    mag_msg = message_magnetization_deg3(A)

    for it in 1:maxiter
        A, Δ, mag_msg = onestep_rs_deg3(A, pᵢ⁰, wᵢ, ϕᵢ, mag_old=mag_msg; ε, 
            showprogress)
        push!(Δs, Δ)
        bd = bond_dims(A)
        if verbose
...
function belief_rs_deg3(A::MPEM2, pᵢ⁰, wᵢ, ϕᵢ; ε=1e-6, showprogress = true)
...
function magnetization_rs_deg3(A::MPEM2, pᵢ⁰, wᵢ, ϕᵢ; ε=1e-6, showprogress = true)
    b = belief_rs_deg3(A, pᵢ⁰, wᵢ, ϕᵢ; ε, showprogress)
    mag = reduce.(-, b)
    return mag
end
